...
class Specification:
    ...
    def __init__(self, source, action, target, r):
        self.spec_s = source
        self.spec_a = action
        self.spec_t = target
        self.sat_r = r

    def is_satisfied(self, source, action, target):
        if target == self.spec_t:
            return True
        else:
            return False

    def check(self, source, action, target):
        if self.is_satisfied(source, action, target):
            return self.sat_r
        else:
            return 0
...
with b0RemoteApi.RemoteApiClient('b0RemoteApi_V-REP-addOn',
    ...
    def learn(args=None):
        ...
        spec = Specification(None, None, 4, lamda * 1.0)
        performance = []
        threshold = gamma ** max_steps
        L = None
        ...
        for l in range(num_episodes):
            ...
            for m in range(max_steps):
                ...
                if USE_MAX_REWARDS:
                    # track maximum of the Return occuring during the episode
                    if r > r_max:
                        r_max = r
                if done or m == max_steps-1:
                    ...
                    # no intermediate Return (i.e., during episode) is given.
                    r = 0
                """

                print("collected max reward in this step: " + str(max(rewards)))
            ...
            performance.append(Return)
            scatter_plot.plot(l, Return, 'b.')
            if (l + 1) % 25 == 0:
                print('c')
                q_tabel.clear()
                q_tabel.imshow(
                ...
                L_l = sum([gamma ** k * r for k, r in enumerate(rewards)]) + lamda * (
                            sum([gamma ** k * c for k, c in enumerate(constraints)]) - threshold)
                L = L_l if L is None else 0.1 * L_l + (1-0.1)*L
        # close()
    ...
    if __name__ == "__main__":
        ...
        if tune:
            ...
            res = gp_minimize(learn, dimensions=[(0.0, 100.0)],
                              x0=[100.0], n_calls=30, n_jobs=-1, verbose=True)
            print(res)
        else:
